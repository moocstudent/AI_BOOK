...
function getPassphrase() {
  if (process.env.AI_BOOK_KEY) return process.env.AI_BOOK_KEY;
  if (existsSync(KEYFILE)) {
    const m = readFileSync(KEYFILE, 'utf8').match(/__AI_BOOK_KEY__\s*=\s*"([^"]+)"/);
    if (m) return m[1];
  }
