 ...
 *   window.__AI_BOOK_KEY__    (firebase-key.local.js  — local passphrase, gitignored)
 *                             or localStorage 'ai_book_key' (set via aiBookUnlock)
 * Exposes:
 *   window.__FIREBASE_READY__  Promise<boolean>  (true once Firebase is initialized)
 *   window.aiBookUnlock(pass)  store passphrase in localStorage + reload (for deploy)
 *
...
(function () {
  ...
  // For deployment without a local key file: run aiBookUnlock("<passphrase>") once.
  window.aiBookUnlock = function (pass) {
    try { localStorage.setItem('ai_book_key', pass); } catch (e) {}
    location.reload();
  };
  ...
  window.__FIREBASE_READY__ = (async function () {
    ...
    let pass = window.__AI_BOOK_KEY__;
    try { pass = pass || localStorage.getItem('ai_book_key'); } catch (e) {}
    if (!pass) {
      window.__FIREBASE_LOCKED__ = true;
      console.warn('[firebase-init] Locked: no passphrase. Add firebase-key.local.js, or run aiBookUnlock("<passphrase>").');
      return false;
    }
    try {
